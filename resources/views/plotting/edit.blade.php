        ...
        <div class="col-lg-10">
            <div class="card stretch stretch-full">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('plotting.update', $plotting->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <select name="tuk_id" class="form-control select2 @error('tuk_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($tuk as $item)
                                        <option value="{{ $item->id }}" {{ old('tuk_id', $plotting->tuk_id) == $item->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <input type="date" name="tanggal" class="form-control @error('tanggal') is-invalid @enderror"
                                    value="{{ old('tanggal', $plotting->tanggal?->format('Y-m-d')) }}" required>
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <div class="row g-2">
                                    <div class="col-6">
                                        ...
                                        <input type="time" name="waktu_asesmen" class="form-control" value="{{ old('waktu_asesmen', $plotting->waktu_asesmen) }}">
                                    </div>
                                    <div class="col-6">
                                        <label class="form-label small text-muted mb-1">Selesai</label>
                                        <input type="time" name="waktu_selesai" class="form-control" value="{{ old('waktu_selesai', $plotting->waktu_selesai) }}">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6 mb-4">
                                ...
                                <input type="text" name="skema_sertifikasi" class="form-control @error('skema_sertifikasi') is-invalid @enderror"
                                    value="{{ old('skema_sertifikasi', $plotting->skema_sertifikasi) }}" required>
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <select name="metode_kegiatan" class="form-control select2 @error('metode_kegiatan') is-invalid @enderror" required>
                                    ...
                                    <option value="Luring" {{ old('metode_kegiatan', $plotting->metode_kegiatan) == 'Luring' ? 'selected' : '' }}>Luring</option>
                                    <option value="Daring" {{ old('metode_kegiatan', $plotting->metode_kegiatan) == 'Daring' ? 'selected' : '' }}>Daring</option>
                                    <option value="Hybrid" {{ old('metode_kegiatan', $plotting->metode_kegiatan) == 'Hybrid' ? 'selected' : '' }}>Hybrid</option>
                                </select>
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <input type="text" name="holding" class="form-control @error('holding') is-invalid @enderror"
                                    value="{{ old('holding', $plotting->holding) }}" required>
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <input type="number" name="jumlah_peserta" class="form-control @error('jumlah_peserta') is-invalid @enderror"
                                    value="{{ old('jumlah_peserta', $plotting->jumlah_peserta) }}" min="1" required>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                ...
                                <input type="email" name="approver_h4" class="form-control @error('approver_h4') is-invalid @enderror"
                                    value="{{ old('approver_h4', $plotting->approver_h4) }}" placeholder="email@perusahaan.com">
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                @if($plotting->spt_h2)
                                <div class="mb-2">
                                    <a href="{{ Storage::url($plotting->spt_h2) }}" target="_blank" class="btn btn-sm btn-outline-primary">
                                        <i class="feather-file me-1"></i>Lihat File Sekarang
                                    </a>
                                </div>
                                @endif
                                <input type="file" name="spt_h2" class="form-control @error('spt_h2') is-invalid @enderror" accept=".pdf,.doc,.docx">
                                <small class="text-muted">Kosongkan jika tidak ingin mengganti file. Format: PDF, DOC, DOCX. Maks 5MB</small>
                                @error('spt_h2')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="col-md-4 mb-4">
                                <label class="form-label">Status</label>
                                <select name="status" class="form-control select2">
                                    <option value="Pending" {{ old('status', $plotting->status) == 'Pending' ? 'selected' : '' }}>Pending</option>
                                    <option value="Disetujui" {{ old('status', $plotting->status) == 'Disetujui' ? 'selected' : '' }}>Disetujui</option>
                                    <option value="Ditolak" {{ old('status', $plotting->status) == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                                </select>
                            </div>
                            <div class="col-md-4 mb-4">
                                <label class="form-label">Tgl Diajukan</label>
                                <input type="date" name="tgl_diajukan" class="form-control" value="{{ old('tgl_diajukan', $plotting->tgl_diajukan?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-4 mb-4">
                                <label class="form-label">Tgl Direspon</label>
                                <input type="date" name="tgl_direspon" class="form-control" value="{{ old('tgl_direspon', $plotting->tgl_direspon?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-6 mb-4">
                                <label class="form-label">Asesor Pengganti</label>
                                <input type="text" name="asesor_pengganti" class="form-control" value="{{ old('asesor_pengganti', $plotting->asesor_pengganti) }}">
                            </div>
                            <div class="col-md-6 mb-4">
                                <label class="form-label">Progres</label>
                                <select name="progres" class="form-control select2">
                                    <option value="Belum" {{ old('progres', $plotting->progres) == 'Belum' ? 'selected' : '' }}>Belum</option>
                                    <option value="Proses" {{ old('progres', $plotting->progres) == 'Proses' ? 'selected' : '' }}>Proses</option>
                                    <option value="Selesai" {{ old('progres', $plotting->progres) == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                                </select>
                            </div>
                            <div class="col-md-6 mb-4">
                                <label class="form-label">Kinerja Asesor</label>
                                <input type="text" name="kinerja_asesor" class="form-control" value="{{ old('kinerja_asesor', $plotting->kinerja_asesor) }}">
                            </div>
                            <div class="col-md-6 mb-4">
                                <label class="form-label">Tgl Pleno</label>
                                <input type="date" name="tgl_pleno" class="form-control" value="{{ old('tgl_pleno', $plotting->tgl_pleno?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-12 mb-4">
                                <label class="form-label">Catatan Asesmen</label>
                                <textarea name="catatan_asesmen" class="form-control" rows="3">{{ old('catatan_asesmen', $plotting->catatan_asesmen) }}</textarea>
                            </div>
                            <div class="col-md-12 mb-4">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="terbit_sertifikat" id="terbit_sertifikat" value="1"
                                        {{ old('terbit_sertifikat', $plotting->terbit_sertifikat) ? 'checked' : '' }}>
