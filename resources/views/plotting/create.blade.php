                ...
                <div class="card-body">
                    <form action="{{ route('plotting.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="peserta_id" class="form-control select2 @error('peserta_id') is-invalid @enderror" required>
                                ...
                                @foreach($peserta as $item)
                                    <option value="{{ $item->id }}" {{ old('peserta_id') == $item->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="lokasi" class="form-control @error('lokasi') is-invalid @enderror" value="{{ old('lokasi') }}" placeholder="Masukkan lokasi plotting" required>
                            @error('lokasi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
